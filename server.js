...
app.post('/cadastro', async (req, res) => {
    ...
    try {
        ...
        db.query('SELECT * FROM usuarios WHERE email = ?', [email], async (err, results) => {
            ...
            // Inserir usuário
            const query = 'INSERT INTO usuarios (nome, email, senha, tipo, telefone) VALUES (?, ?, ?, ?, ?)';
            db.query(query, [nome, email, senhaHash, tipo || 'aluno', telefone || null], (err, result) => {
                if (err) {
...
app.get('/usuarios/:id', (req, res) => {
    ...
    db.query('SELECT id, nome, email, telefone, tipo, data_criacao FROM usuarios WHERE id = ?', [id], (err, results) => {
        if (err) {
...
app.get('/oficinas', (req, res) => {
    db.query('SELECT * FROM oficinas ORDER BY data_inicio DESC', (err, results) => {
        if (err) {
            return res.status(500).json({ erro: 'Erro no servidor' });
        }
        res.json(results);
...
app.post('/oficinas', (req, res) => {
    const { titulo, descricao, instrutor, data_inicio, data_fim, vagas, categoria } = req.body;

    if (!titulo || !descricao || !instrutor || !data_inicio) {
        return res.status(400).json({ erro: 'Campos obrigatórios faltando' });
    }

    const query = 'INSERT INTO oficinas (titulo, descricao, instrutor, data_inicio, data_fim, vagas, categoria) VALUES (?, ?, ?, ?, ?, ?, ?)';

    db.query(query, [titulo, descricao, instrutor, data_inicio, data_fim, vagas || 20, categoria], (err, result) => {
        if (err) {
...
app.put('/oficinas/:id', (req, res) => {
    ...
    const query = 'UPDATE oficinas SET titulo = ?, descricao = ?, instrutor = ?, data_inicio = ?, data_fim = ?, vagas = ?, categoria = ? WHERE id = ?';

    db.query(query, [titulo, descricao, instrutor, data_inicio, data_fim, vagas, categoria, id], (err, result) => {
        if (err) {
...
app.post('/inscricoes', (req, res) => {
    ...
    if (!usuario_id || !oficina_id) {
        return res.status(400).json({ erro: 'Usuario_id e oficina_id são obrigatórios' });
    }

    // Verificar se já existe inscrição
    db.query('SELECT * FROM inscricoes WHERE usuario_id = ? AND oficina_id = ?', [usuario_id, oficina_id], (err, results) => {
        if (err) {
            return res.status(500).json({ erro: 'Erro no servidor' });
        }

        if (results.length > 0) {
            return res.status(400).json({ erro: 'Você já está inscrito nesta oficina' });
        }

        // Criar inscrição
        const query = 'INSERT INTO inscricoes (usuario_id, oficina_id) VALUES (?, ?)';
